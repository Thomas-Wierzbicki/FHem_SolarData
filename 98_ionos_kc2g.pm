...
sub ionos_kc2g_updateStationList {
  eval {
    ...
    Log3 undef, 3, "ionos_kc2g: stationId-AttrList gesetzt mit $list";
  };
  if ($@) {
    Log3 undef, 2, "ionos_kc2g: Fehler beim Laden der stationId-Liste: $@";
  }
}
>
=cut
...
sub ionos_kc2g_Attr {
  ...
  # Nur auf Änderung des interval-Attributs reagieren
  if ($cmd eq "set" && $attrName eq "interval") {
    Log3 $name, 4, "ionos_kc2g: interval changed to $attrValue, restarting timer";
...
sub ionos_kc2g_Initialize {
  ...
  #--------Neu---------
  $hash->{AttrFn} = 'ionos_kc2g_Attr';
  #
  
  $hash->{SetList}  = 'update';
  #
  #ionos_kc2g_updateStationList($h);
  #
}
...
sub ionos_kc2g_Define {
  ...
  $hash->{INTERVAL} = AttrVal($hash->{NAME}, 'interval', 90);
...
# updateIonosState($hash);

  updateIonosState($hash->{NAME});
